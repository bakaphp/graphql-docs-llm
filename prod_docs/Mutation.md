...
**Retorno:** `Organization!`\
Merge `source` Organization into `target` — rewrites Scribe + Guild FKs then
soft-deletes source. Used to clean up duplicates created by the
auto-resolve-vendor PDF-ingest flow.
